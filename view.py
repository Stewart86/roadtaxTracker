...
class Multicolumn_Listbox(Frame):
    ...
    def __init__(self, master, columns, data=None, command=None, sort=True,
                 ...
                 style=None, height=None, padding=None,
                 adjust_heading_to_content=False, stripped_rows=None,
                 selection_background=None, selection_foreground=None,
        ...
        self.title = Label(self.inputframe, font=(
        ...
        self.interior = Treeview(master, columns=columns, height=20,
                                 **treeview_kwargs,
        ...
        for i in range(0, self._number_of_columns):
            ...
            if sort:
                self.interior.heading(
                    i, text=columns[i], anchor=heading_anchor, 
    ...
    def configure_column(self, index, width=None, minwidth=None, anchor=None,
                         stretch=None):
        kwargs = {}
    ...
    def update_row(self, index, data):
        ...
        if len(data) == len(self._columns):
            ...
            raise ValueError(
                "The multicolumn listbox has only %d columns" % self._number_of_columns)
    ...
    def delete_row(self, index):
        ...
        if self._stripped_rows:
            for i in range(index, self._number_of_rows):
                self.interior.tag_configure(
                    list_of_items[i+1], background=self._stripped_rows[i % 2])
    ...
    def insert_row(self, data, index=None):
        ...
        if self._stripped_rows:
            ...
            self.interior.tag_configure(
                item_ID, background=self._stripped_rows[index % 2])

            for i in range(index+1, self._number_of_rows):
                self.interior.tag_configure(
                    list_of_items[i], background=self._stripped_rows[i % 2])

    def column_data(self, index):
        return [self.interior.set(child_ID, index) for child_ID
                in self.interior.get_children('')]
    ...
    def set_selection(self, indices):
        ...
        self.interior.selection_set(
            " ".join(list_of_items[row_index] for row_index in indices))
...
chkbox1 = Checkbutton(mc.inputframe, text="Informed",
                      variable=informed, bg='lavender')
chkbox2 = Checkbutton(mc.inputframe, text="Inspected",
                      variable=inspected, bg='lavender')
chkbox3 = Checkbutton(mc.inputframe, text="Renewed",
